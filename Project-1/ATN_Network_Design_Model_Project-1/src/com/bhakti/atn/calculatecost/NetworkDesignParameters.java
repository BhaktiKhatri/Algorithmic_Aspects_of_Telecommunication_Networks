package com.bhakti.atn.calculatecost;

import java.util.ArrayList;
import java.util.Collections;

import com.bhakti.atn.dijkstra.Graph;
...
public class NetworkDesignParameters {
	...
	public void initializeUnitCost() {
		...
		for(int i = 0; i < numberOfNodes; i++){
	        ...
	        for (int p=0; p < k; p++) {
	            //System.out.println(""+i+"---"+list.get(p));
	...
	public void initializeTrafficDemand() {
		...
		int studentID1[] = {2, 0, 2, 1, 3, 0, 9, 3, 1, 8,2, 0, 2, 1, 3, 0, 9, 3, 1, 8,2, 0, 2, 1, 3, 0, 9, 3, 1, 8};
	...
	public int calculateNetworkDensity(){
		...
		for(int i =0; i< numberOfNodes; i++){
			for(int j =0; j< numberOfNodes; j++){
				...
				if (i != j & (unitMinPathCost[i][j] == 0 || (unitMinPathCost[i][j] != 1 & unitMinPathCost[i][j] != 300))){
					count ++;
